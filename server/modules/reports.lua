...
AddEventHandler("MDT:Server:RegisterCallbacks", function()
    plsr.Callbacks:RegisterServerCallback("MDT:Search:report", function(source, data, cb)
        local char = plsr.Fetch:CharacterSource(source)
		if CheckMDTPermissions(source, false) or (char:GetData("Attorney") and data.isAttorney) then
			cb(plsr.MDT.Reports:Search(data.term, data.reportType, data.page, data.perPage, data.isAttorney, data.evidence))
		else
			cb(false)
		end
    end)

    plsr.Callbacks:RegisterServerCallback("MDT:Search:report-evidence", function(source, data, cb)
		if CheckMDTPermissions(source, false) then
			cb(plsr.MDT.Reports:SearchEvidence(data.term))
		else
			cb(false)
		end
    end)

    plsr.Callbacks:RegisterServerCallback("MDT:Create:report", function(source, data, cb)
        local char = plsr.Fetch:CharacterSource(source)
		if CheckMDTPermissions(source, false) then
			data.doc.author = {
			...
			cb(plsr.MDT.Reports:Create(data.doc))
        else
            cb(false)
        end
    end)

    plsr.Callbacks:RegisterServerCallback("MDT:Update:report", function(source, data, cb)
        local char = plsr.Fetch:CharacterSource(source)
		if char and CheckMDTPermissions(source, false) then
			cb(plsr.MDT.Reports:Update(data.id, char, data.report))
        else
            cb(false)
        end
    end)

    plsr.Callbacks:RegisterServerCallback("MDT:Delete:report", function(source, data, cb)
		if CheckMDTPermissions(source, true) then
			cb(plsr.MDT.Reports:Delete(data.id))
        else
            cb(false)
        end
    end)

    plsr.Callbacks:RegisterServerCallback("MDT:View:report", function(source, data, cb)
        local char = plsr.Fetch:CharacterSource(source)
		if CheckMDTPermissions(source, false) or char:GetData("Attorney") then
			cb(plsr.MDT.Reports:View(data))
        else
			cb(false)
		end
    end)
