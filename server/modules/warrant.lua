...
_MDT.Warrants = {
	...
	Create = function(self, report, suspect, notes, author)
		local id = MySQL.insert.await("INSERT INTO mdt_warrants (title, report, suspect, notes, creatorSID, creatorName, creatorCallsign, expires) VALUES (?, ?, ?, ?, ?, ?, ?, FROM_UNIXTIME(?))", {
			...
			author.Callsign,
			os.time() + (60 * 60 * 24 * config.Warrants.defaultDurationDays)
		})
...
AddEventHandler("MDT:Server:RegisterCallbacks", function()
	plsr.Callbacks:RegisterServerCallback("MDT:Search:warrant", function(source, data, cb)
		cb(plsr.MDT.Warrants:Search(data.term, data.page, data.perPage))
	end)

	plsr.Callbacks:RegisterServerCallback("MDT:View:warrant", function(source, data, cb)
		cb(plsr.MDT.Warrants:View(data))
	end)

	plsr.Callbacks:RegisterServerCallback("MDT:Update:warrant", function(source, data, cb)
		local char = plsr.Fetch:CharacterSource(source)

		if char and CheckMDTPermissions(source, false) then
			cb(plsr.MDT.Warrants:Update(data.id, data.state))
		else
			cb(false)
		end
	end)
