...
AddEventHandler("MDT:Server:RegisterCallbacks", function()
	plsr.Callbacks:RegisterServerCallback("MDT:Create:BOLO", function(source, data, cb)
		...
		if CheckMDTPermissions(source, false, 'police') then
			...
			plsr.MDT.Misc.Create:BOLO(data.doc)
			cb(true)
		else
			cb(false)
		end
	...
	plsr.Callbacks:RegisterServerCallback("MDT:Delete:BOLO", function(source, data, cb)
		if CheckMDTPermissions(source, false, 'police') then
			cb(plsr.MDT.Misc.Delete:BOLO(data.id))
		else
			cb(false)
		end
	end)

	plsr.Callbacks:RegisterServerCallback("MDT:Create:charge", function(source, data, cb)
		if CheckMDTPermissions(source, true) then
			data.doc.active = true
			cb(plsr.MDT.Misc.Create:Charge(data.doc))
		else
			cb(false)
		end
	end)

	plsr.Callbacks:RegisterServerCallback("MDT:Update:charge", function(source, data, cb)
		if CheckMDTPermissions(source, true) then
			cb(plsr.MDT.Misc.Update:Charge(data.doc.id, data.doc))
		else
			cb(false)
		end
	end)

	plsr.Callbacks:RegisterServerCallback("MDT:Delete:charge", function(source, data, cb)
		if CheckMDTPermissions(source, true) then
			cb(plsr.MDT.Misc.Delete:Charge(data.doc.id))
		else
			cb(false)
		end
	end)

	plsr.Callbacks:RegisterServerCallback("MDT:Create:notice", function(source, data, cb)
		local char = plsr.Fetch:CharacterSource(source)
		if char and CheckMDTPermissions(source, {
			...
			cb(plsr.MDT.Misc.Create:Notice(data.doc))
		else
			cb(false)
		end
	end)

	plsr.Callbacks:RegisterServerCallback("MDT:View:notice", function(source, data, cb)
		cb(plsr.MDT.Misc.View:Notice(data))
	end)

	plsr.Callbacks:RegisterServerCallback("MDT:Delete:notice", function(source, data, cb)
		if CheckMDTPermissions(source, {
			'PD_HIGH_COMMAND',
			'SAFD_HIGH_COMMAND',
			'DOC_HIGH_COMMAND',
		}) then
			cb(plsr.MDT.Misc.Delete:Notice(data.id))
		else
			cb(false)
		end
	end)
