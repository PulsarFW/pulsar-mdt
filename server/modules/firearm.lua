...
AddEventHandler("MDT:Server:RegisterCallbacks", function()
	plsr.Callbacks:RegisterServerCallback("MDT:Search:firearm", function(source, data, cb)
		if CheckMDTPermissions(source, false) then
			cb(plsr.MDT.Firearm:Search(data.term or ""))
		else
			cb(false)
		end
	end)

	plsr.Callbacks:RegisterServerCallback("MDT:View:firearm", function(source, data, cb)
		if CheckMDTPermissions(source, false) then
			cb(plsr.MDT.Firearm:View(data))
		else
			cb(false)
		end
	end)

	plsr.Callbacks:RegisterServerCallback("MDT:Create:firearm-flag", function(source, data, cb)
		local char = plsr.Fetch:CharacterSource(source)
		if char and CheckMDTPermissions(source, false) then
			cb(plsr.MDT.Firearm.Flags:Add(data.parentId, data.doc, {
	...
	plsr.Callbacks:RegisterServerCallback("MDT:Delete:firearm-flag", function(source, data, cb)
		if CheckMDTPermissions(source, false) then
			cb(plsr.MDT.Firearm.Flags:Remove(data.parentId, data.id))
		else
			cb(false)
		end
	end)
