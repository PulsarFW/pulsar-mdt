<script lang="ts">
	...
	let { selectedSid = $bindable(null) }: { selectedSid: number | null } = $props();
	...
	async function search() {
		...
		searched = true;
		page = 1;
		results = await Nui.searchPeople(term.trim());
		loading = false;
	}
...
<div class="search-pane">
	<form class="bar" onsubmit={(e) => (e.preventDefault(), search())}>
		<input type="text" placeholder="Search Name or State ID" bind:value={term} />
		{#if term}
	...
	<div class="results">
		{#if loading}
			<Loader />
		{:else if !searched}
			<p class="hint">Search for a person by name or state ID.</p>
		{:else if pageResults.length === 0}
			<p class="hint">No results found.</p>
		{:else}
			{#each pageResults as person (person.SID)}
				<button type="button" class="row" class:active={selectedSid === person.SID} onclick={() => (selectedSid = person.SID)}>
					<div class="avatar"><Icon name="user-large" size="1.6vmin" /></div>
					<div class="info">
						<span class="name">{person.First} {person.Last}</span>
						<small>State ID: {person.SID} - DOB: {formatDate(person.DOB)}</small>
					</div>
				</button>
			{/each}
		{/if}
